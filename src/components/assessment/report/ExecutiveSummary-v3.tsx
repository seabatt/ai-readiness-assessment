...
export default function ExecutiveSummary({ 
  ...
  return (
    <div className="max-w-5xl mx-auto mb-16">
      ...
      <div className="text-center">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-bg-card border border-brand-secondary/10">
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <svg 
                key={i}
                className={`w-4 h-4 ${i < Math.round(roiResult.confidence / 20) ? 'text-accent-green' : 'text-brand-tertiary'}`}
                fill="currentColor" 
                viewBox="0 0 20 20"
              >
          ...
          <span className="text-sm text-text-secondary">
            {roiResult.confidence}% Confidence Score
          </span>
